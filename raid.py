...
██║  ██║ ██║  ██║ ██║ ██████╔╝
╚═╝  ╚═╝ ╚═╝  ╚═╝ ╚═╝ ╚═════╝
{C.RESET}{C.WHITE}   Rapid Asset & Infrastructure Discovery
{C.RED}   {C.BOLD}Built by Chiron{C.RESET}{C.WHITE} · Offensive Security Toolkit
{C.RESET}{C.RED}{'─' * 46}{C.RESET}
"""
...
# Different operating systems ship with different default TTL values.
# We can't observe the *original* TTL directly (it decreases by 1 per hop),
# so we compare the received TTL against the nearest common default and
# infer how many hops away the target is. This is the same first-pass
# heuristic Nmap and other scanners use before deeper stack fingerprinting.
TTL_DEFAULTS = [
    (255, "Cisco/Solaris/older Unix"),
    (128, "Windows"),
    (64, "Linux/Unix/macOS"),
]

def guess_os_from_ttl(ttl):
    for default, os_name in TTL_DEFAULTS:
        if ttl <= default:
            hops = default - ttl
            return os_name, default, hops
    return "Unknown", None, None


def detect_os(target, timeout):
    try:
    ...
    try:
        resp = sr1(IP(dst=target) / ICMP(), timeout=timeout, verbose=0)
        if resp is None:
            return None
        ttl = resp.ttl
        os_name, default, hops = guess_os_from_ttl(ttl)
        return {"ttl": ttl, "guessed_default": default,
                "estimated_hops": hops, "os_guess": os_name}
    except Exception:
        return None
...
def main():
    ...
    parser.add_argument("--os", action="store_true",
                         help="Attempt basic OS detection via ICMP TTL "
                              "fingerprinting (requires scapy; best-effort, "
                              "not as precise as Nmap's -O)")
    ...
    os_result = None
    if args.os:
        os_result = detect_os(target_ip, args.timeout)
        if os_result:
            print(f"{C.MAGENTA}[*] OS Guess : {os_result['os_guess']} "
                  f"(TTL={os_result['ttl']}, ~{os_result['estimated_hops']} "
                  f"hop(s) from nearest default {os_result['guessed_default']}){C.RESET}")
        else:
            print(f"{C.YELLOW}[!] OS detection failed. This usually means one "
                  f"of the following:{C.RESET}")
            print(f"{C.YELLOW}    - RAID wasn't run with sudo (raw ICMP sockets "
                  f"need root){C.RESET}")
            print(f"{C.YELLOW}    - The target is unreachable or blocks ICMP "
                  f"(common with firewalls){C.RESET}")
            print(f"{C.YELLOW}    - scapy isn't installed in this environment"
                  f"{C.RESET}")
        print(f"{C.WHITE}{'-'*55}{C.RESET}")
